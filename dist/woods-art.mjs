import {heldItem} from './market-art.mjs?v=design-fit-2';
...
export function drawDwarf(ctx,art,x,y,{time=0,walk=0,moving=false,face=1,weapon=0,armor=0,boots=0,attack=null,dodge=0,hurt=0,scale=1,marketGear=null,fitting=false,armAngle:poseArm=null}={}){
 ...
 function leg(x,a){ctx.save();ctx.translate(x,-61);ctx.rotate(a);if(boots&&!marketGear?.boots)ctx.filter=`hue-rotate(${boots*18}deg) brightness(${1+boots*.1})`;ctx.save();if(x<0)ctx.scale(-1,1);part(3,-20,0,40,65);ctx.restore();if(marketGear?.boots){const im=marketGear.boots,half=im.width/2;ctx.drawImage(im,x<0?0:half,0,half,im.height,-29,9,58,58);}ctx.restore();}
 ...
 ctx.restore();
part(0,-65,-244,130,141);if(marketGear?.helmet){const im=marketGear.helmet,w=130,h=w*im.height/im.width,eye=[.62,.59,.63,.64,.67,.68,.65,.76][marketGear.helmetTier||0];ctx.drawImage(im,8-w/2,-198-h*eye,w,h);}
