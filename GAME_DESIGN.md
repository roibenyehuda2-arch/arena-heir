...
## Articulated fighter rig — 2026-09-14

This is a visual repair of the fighters themselves, not a gameplay change. Combat rules, economy, progression, saves, town behaviour and hosting are untouched.

The previous rig forced every sprite into a fixed rectangle: the mage's 99x292 arm was drawn at 40x80 and its 173x334 leg at 50x73, so no two parts shared a scale and the figures read as assembled from spares. Mage and archer drew a single arm, and both legs reused one unmirrored sprite, so the feet pointed the same way. Worn gear was the shop's own inventory illustration pasted onto the body: a 76px helmet icon over a 130px head, a boots picture cut at its midpoint and stretched over fully drawn bare feet, sleeved armor artwork with naked animated arms drawn on top.

`dist/fighter-rig.mjs` replaces it. Each class has an explicit skeleton — head, torso, two shoulder joints, two hip joints, a grip fraction along the arm — and every part is drawn at the aspect ratio of its own artwork, scaled to a per-class target height. The far arm and far leg hang from their own joints with their own swing and a darkening pass for depth. Mirroring them was tried first and rejected: these are side-view sprites, so a flipped foot points backwards mid-stride. Ground sits at y=0 and the crown at -248 for all classes, so every existing call site keeps its scale.

Worn equipment keeps the shop's own painted illustrations; only their placement changed. A first attempt replaced them with shapes drawn in code, and both the user and an independent review rejected it: the flat gradient gear fought the painted heads, beards and weapons, and the figure lost its legs and arms under it. The artwork was never the defect. Helmets are now sized from a per-class skull box, which is separate from the head sprite's bounding box because that box includes the dwarf's beard and the mage's hair, so the painted helm's face opening lands on the face. The atlas ships one helm set for all three classes — it has no wizard hats or hoods yet — so the classes differ only in fit. Shoulders and boots ship as left/right pairs in one cell and are split so each half goes on its own joint, with the boot's sole on the foot instead of hovering up the shin. The shield is drawn after the torso, because carried on the off hand it belongs in front of the body, not behind it. Body armor is scaled to the torso.

What a hand holds is drawn before the arm, so the fist closes over the haft rather than the weapon floating in front of it.

Held weapons keep their illustrations, because that ladder is the visible reward for spending. The old fitting width cap of 105px shrank exactly the expensive wide weapons it should have shown off; weapons now scale by height across eight steps and clamp only enough to stay off the face and inside the stage. Tall hafts are gripped low on the shaft, broad weapons nearer their balance point, and a ranger's bow is stood upright rather than carried flat.

Two atlas-slicing defects were corrected at the same time, because they cut the artwork before the rig ever saw it. `class-parts.webp` was sliced in even thirds, putting a row line at y=724 that removed 39-51px from the top of every weapon in the bottom row; the real gutters are at 350 and 670. `starter-parts.webp` had the same problem, gluing a 9px strip of the archer's green tunic onto every mage cell and shaving the archer row; its real row tops are 0, 348 and 666. `starter-bodies.webp` is no longer drawn on the fighter at all: it is three headless torsos in white and brown that contradict the approved design, and its first column boundary cut through the first body.

The shop's fitting room also cut the fighter off at the knees on every phone width: its label bar is opaque and grows to 44px, while the fighter stood a fixed 36px off the panel. The floor is now derived from that label, so the whole figure clears it.

Limits: the atlases provide one arm and one leg per class, so the far limb is the same sprite at a different joint rather than separate artwork. There is one painted helm set rather than class-specific hats and hoods, so a mage and an archer wear the same helm shape at a different fit. The archer's head sprite still carries the flower crown from the older art, and the starter tunic, arm and leg sprites are the ones already shipped.
