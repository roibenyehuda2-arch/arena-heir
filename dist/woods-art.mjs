...
export function drawDwarf(ctx,art,x,y,{time=0,walk=0,moving=false,face=1,weapon=0,armor=0,boots=0,attack=null,dodge=0,hurt=0,scale=1}={}){
 ...
 function leg(x,a){ctx.save();ctx.translate(x,-61);ctx.rotate(a);if(boots)ctx.filter=`hue-rotate(${boots*18}deg) brightness(${1+boots*.1})`;part(3,-19,0,40,65);ctx.restore();}
 leg(-21,-gait);ctx.translate(0,-bob);
 ctx.save();ctx.translate(-36,-142);ctx.rotate(.15+gait*.45);part(2,-18,0,38,80);ctx.restore();
 leg(17,gait);part(ARMORS[armor].part,-51,-152,102,106);
 let armAngle=-.12+gait*.5;
 ...
 if(armor)part(armor===1?9:10,19,-158,55,48);
 if(armor===2)part(11,-62,-254,122,94);
 ctx.save();ctx.translate(34,-139);ctx.rotate(armAngle);ctx.translate(4,60);ctx.rotate(.40);part(WEAPONS[weapon].part,-36,-124,80,155);ctx.restore();
 ctx.restore();
}
