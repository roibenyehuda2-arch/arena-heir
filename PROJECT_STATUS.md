...
## Short bouts and Nameless Trial — 2026-09-12

Built on synchronized main 16660f160cd84be56ccb7cea43d0850cafed962c. Added first-blood ordinary battles with visible protection, full-health tournament rules, shorter exchanges, four contextual control categories, fresh production RNG seeds and save-compatible legacy battles. Added the skippable class introduction and retryable Gatekeeper trial, with a once-only 150 gold / three-point starter grant. Leaving a result clears it from the save so reloading after shopping returns to town. Existing art, named rivals, six AI styles, crowd bonuses, sound, equipment, tournament and unlock rules are preserved.

QA: complete npm test and README syntax commands pass. Added armor absorption/exact-break/first-wound checks, tournament continuation, old battle saves, all-class trial retry/skip/reward/save tests and 300 first-rival simulations. With starter gear and a simple highest expected damage policy, each class has a five-decision median; p90 is eight (dwarf), seven (ranger), five (mage). This is a bounded simulation, not a promise for all builds or proof of enjoyment. Existing full class-to-crown regressions pass.

Actual Chrome browser QA reviewed the intro at 320×568 and 390×660, trial move/melee/result flow at phone dimensions, and combat at 430×740 and 1024×768. Fixed covered legs, overlapping melee bodies, the long Taunt label, practice exit covering the rival name, and oversized trial result title. Played the dwarf trial to victory, checked skip-to-town, purchased Ironbark axe, reloaded to verify 105 gold and owned equipment, then entered practice with the purchased axe. Shop preview and the upgrade comparison remain readable. No physical iPhone/Safari or real touch/performance test was available. Read-only subagent review informed rule/migration edge cases.

Remaining: subjective combat feel and class balance need player feedback; simulations favor ranger/mage over an unequipped dwarf. The wider progression/catalog redesign and unique rival character animation are not completed by this slice. Prepared for the authorized GitHub Pages release; deployment verification will be recorded after publication. Temporary QA wrapper must be removed before release.
Verified publication: release `2750c3009a0c73a14ed5c3038f89d71a907828ee` on main. Actions run `34692428121` completed validation and deployment successfully. Live GitHub Pages returned HTTP 200 and byte-for-byte matches for index.html, duel.js, duel-engine.mjs, duel.css, arena-art.mjs and woods-art.mjs. The temporary QA wrapper was removed and the local preview stopped. Main was clean and synchronized to the release. This subsequent handoff note is documentation-only [skip ci].


## Combat distance and decisive finish correction — 2026-09-12

Built on ef97c17bc70d280fc277eed8ed3006a1351851ad. Replaced range-dependent close-up camera with a fixed wide arena view and smaller fighters. Preserved the arena artwork's horizontal field of view. Added a shared presentation module for camera, health display and action poses/timing, and explicit arm control in existing dwarf/ranger/mage art. Moved visible health loss and hit sound to the contact beat; added short wind-up, body lean, recoil and miss movement. Added a persistent fallen/motionless loser pose with VICTORY/DEFEATED and a working Continue/Claim reward control before settlement.

New fights now end at zero normal HP, removing the confusing first-blood/armor-pool combination. Armor damage reduction, ordinary defeat penalties, tournament elimination, shops, trial grant and master unlocks remain. Existing mid-battle rules remain readable and resumable. Prevented forfeiting an already finished battle.

Validation: full npm test and README syntax checks pass. New tests cover fixed camera size across distance and four screen sizes, zero-HP finishes, contact poses, end-state display and once-only reward settlement. Existing all-class crown simulations pass; 300 unupgraded first-rival simulations have median player decisions 7/5/5 (dwarf/ranger/mage). This is a bounded policy simulation, not a general balance or fun claim. Browser QA uses Chrome phone-size frames and explicit temporary low-HP test scenarios for deterministic win/loss visual checks; it is not physical iPhone/Safari testing. Fixed a real pointer-events defect that prevented the result button from receiving clicks. The temporary QA page must be removed before deployment. Publication verification follows after the authorized GitHub Pages release.
Browser checks completed: normal movement/heavy exchange at 390×660; deterministic enemy fall, zero HP and Claim reward granting level 2 / 47 gold; player fall and working Continue to ordinary-defeat result at 320×568; mage Lightning and rival damage at 430×740; fixed wide desktop camera at 1024×768. Screenshots inspected. The only errors in the sampled browser log were browser-extension metadata errors, not game errors. Temporary fixture removed before release. The art remains modular sprite animation, not newly drawn frame-by-frame character animation.
Verified publication: release `16aeec7959debb89f532157c66d32ca3f28823b3` on main. Actions run `34707722499` passed validation and deployment. Live GitHub Pages returned HTTP 200 and byte-for-byte matches for index.html, duel.js, duel-engine.mjs, duel-presentation.mjs, duel.css, arena-art.mjs and woods-art.mjs. The preview was stopped and main synchronized cleanly. This follow-up handoff note changes documentation only [skip ci].
# Market display iteration — 2026-09-13 (publication pending)

User confirmed the other account has stopped. Clean main fetched at 5a9eeeb with no divergence. Enlarged merchant independently of fitting fighter, separated their positions and reduced fitting scale for weapon clearance. Phone stock is a three-column vertical display (two columns below 360px), with purchase details above stock and wrapping equipment tabs. Updated entry asset version to market-display-2. No catalog, combat, saves or approved character artwork changed.

Checks: npm test, node --check dist/duel.js and git diff --check pass. Chrome iframe visual inspection at 390x740 and 320x740, dwarf/mage weapon displays; after reload mage display inspected with final layout. Browser purchase verified 150 to 105 gold; ranged tab shows seven pieces; 390 shelf has equal client/scroll widths. No physical iPhone/Safari test or final desktop visual pass. Temporary local QA page removed. Approved starter artwork integration, class-specific hats/armor art, helmet fit and town proportions remain pending. Publication is not yet verified; do not treat this entry as a live release.
# Phone town proportions — 2026-09-14

The preceding market release de6097daa9d13ddd176ab6e52040bd3043583c2d was published via the authenticated GitHub connector. Actions run 34786548094 succeeded and the live entry contained market-display-2. Local CLI push has no credentials; use the authorized connector for publication, not repeated credential prompts.

This town-only iteration removes the stretched foreground strip, uses the illustration's native 3:2 phone stage, places the fighter in that same coordinate space, reduces fighter size, replaces opaque signs with compact text over entrances, preserves 44px touch targets and prevents hover displacement. The next-prize panel follows the scene. Existing character art and equipment fitting remain pending separate work.

Validation: npm test and diff checks passed. Chrome visual QA at 320/390/430 x 740; all three shop destinations clicked; Weaponsmith arrival confirmed in DOM, then browser inspection timed out before the other arrival headings could be confirmed. No physical Safari check. Desktop keeps its prior layout. Temporary QA page removed. Release verification pending.
# Starter rig — 2026-09-14

Added starter-parts.webp derived from the approved dwarf/mage/archer concept with separate shirt, bare arm and shorts/bare-foot leg pieces. Production market costumes provide these pieces to both rigs; no-armor figures no longer use torso-only cutouts on old armored limbs. Zero boots now means bare feet. Retained existing heads and animation joints; armor purchases still overlay the base. All production imports versioned starter-rig-1. This is an articulated adaptation of the approved drawing, not an exact full-body copy. Helmet fitting and class-specific accessory catalogs remain pending.

Checks: full npm test passes. Chrome rendering fixture visually inspected all three classes in fitting, animated walking and purchased body/boots states; fixture removed before publication. This turn did not include a fresh physical phone test or a full combat browser playthrough. Prior town release f0ab762c8aaa444ee47d3078644d689bb3699530 completed Actions 34809477881 successfully; live entry/CSS/JS matched. Current starter release verification pending.
