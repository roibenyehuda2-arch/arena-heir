...
`dist/fighter-rig.mjs` replaces it. Each class has an explicit skeleton — head, torso, two shoulder joints, two hip joints, a grip fraction along the arm — and every part is drawn at the aspect ratio of its own artwork, scaled to a per-class target height. The far arm and far leg are mirrored, so the feet point outward and the body reads as one figure. Ground sits at y=0 and the crown at -248 for all classes, so every existing call site keeps its scale.

Worn equipment is painted as fitted layers anchored to those joints rather than blitted from the shop. Helmets fit a per-class skull box, which is separate from the head sprite's bounding box because that box includes the dwarf's beard and the mage's hair; the face is never covered. Headgear is class-shaped: a dwarven helm with a nose guard and horns at high tiers, a wizard's pointed brim, an archer's hood. Body armor spans the real shoulder joints as plate, jerkin or robe and brings a matching sleeve and bracer with it, so an armored fighter never shows a bare arm over a plated chest. Boots enclose each foot separately. Shields hang from the off hand. Eight tier palettes carry the progression from leather through steel, gold, ember, tide, storm and dragon.
...
Limits: the atlases provide one arm and one leg per class, so the far limb is a mirror rather than separate artwork. Worn equipment is procedural, so it reads as fitted gear in the tier's colours rather than as the exact illustration on the shop shelf. The archer's head sprite still carries the flower crown from the older art, and the starter tunic, arm and leg sprites are the ones already shipped.
