...
Worn helmets, shoulders, body armor, boots and shields use the shop's own painted illustrations, fitted to those joints: helmets sized from a per-class skull box so the face stays visible, shoulder and boot pairs split so each half goes on its own joint, and the shield carried in front of the body. What a hand holds is drawn before the arm, so the fist closes over it. Held weapons scale by height across eight tiers, so an expensive weapon looks expensive.

Validation adds `node --check dist/fighter-rig.mjs`. The renderer geometry test now supplies the real measured atlas dimensions and asserts anatomy (two arms and two legs, hung from separate joints), natural aspect ratios for every part, footing on the ground line, weapon grip clearance from the face, stage fit at 320/360/390/430, and a visible tier-1 to tier-7 weapon size progression.
